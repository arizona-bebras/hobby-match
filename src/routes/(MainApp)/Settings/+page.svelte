...
<main>
  ...
  >
    ...
    {:else if selectedWidget == 'todo'}
      ...
      {#each tasksNumbers as task (task.id)}
        <Task onRemove={() => removeTask(task.id)} taskOrder={task.id} />
      {/each}
      <button
        onclick={() =>
          (tasksNumbers = [...tasksNumbers, { id: tasksNumbers.length + 1 }])}
        id="taskCreateButton"
        type="button">Добавить Цель</button
      >
    ...
    {:else if selectedWidget == 'text'}
      <p>Текст</p>
      <textarea class="input-widget-text" id="input-widget-text" name="text"
      ></textarea>
    {/if}
  ...
  {#each widgets as widget}
    {#if widget.widget.data.type == 'text'}
      <div
        class={widget.deleteStatus
          ? 'widget-container deleted'
          : 'widget-container'}
      >
        <TextWidget data={widget.widget.data} />

        <button
          style="margin-right:15px"
          onclick={() => showChangeWidgetField(widget)}
        >
      ...
      {#if widget.changeStatus == true}
        <form
          method="POST"
          use:enhance={() => updateWidget(widget)}
    ...
    {:else if widget.widget.data.type == 'audio'}
      <div
        class={widget.deleteStatus
          ...
          : 'widget-container'}
      >
        <AudioWidget link={widget.widget.data.link} />
        <button class="ch-btn" onclick={() => deleteWidget(widgets, widget)}
          >X</button
        >
        <button
    ...
    {:else if widget.widget.data.type == 'video'}
      <div
        class={widget.deleteStatus
          ...
          : 'widget-container'}
      >
        <VideoWidget data={widget.widget.data} />
        <button class="ch-btn" onclick={() => deleteWidget(widgets, widget)}
          >X</button
        >
        <button
    ...
    {:else if widget.widget.data.type == 'progress_bar'}
      <div
        class={widget.deleteStatus
          ...
          : 'widget-container'}
      >
        <ProgressWidget data={widget.widget.data} />
        <button class="ch-btn" onclick={() => deleteWidget(widgets, widget)}
          >X</button
        >
        <button
