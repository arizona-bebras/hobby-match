...
<!--  </div>-->
  {#each tasks as task}
    ...
    <label class="flex items-center space-x-2">
      <input type="radio" name="radio-direct" bind:group={selected} value={task[0]} class="appearance-none rounded-full size-5.25 border-2 border-[#D9D9D9]
       cursor-pointer checked:bg-accent checked:border-accent peer" disabled={selected != ""}/>
    ...
    {#if (selected != "")}
      <div class="ml-7.25">
        <Progress value={task[1]} max={votes} height="h-1" meterBg={selected === task[0]? "bg-accent":"bg-black/40"} trackBg="bg-[#D9D9D9]"/>
      </div>
    {/if}
  {/each}
