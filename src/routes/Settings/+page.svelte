...
<main class="flex flex-col bg-white flex-1 items-start text-black">
  ...
  <select class="select" bind:value={selectedGame}>
    {#each JSON.parse(data.games) as info}
      <option value='{info.name}, {Math.trunc(info.playtime_forever/60)}, {info.appid}, {info.img_icon_url}'>{info.name}</option>
    {/each}
  </select>
  <p>selected game is: {selectedGame ? selectedGame: 'waiting...'}</p>
    <div class="p-[15px] w-full">
      <div class="flex gap-[12px] bg-[#c2c2c2] p-[10px] rounded-[20px] border-[2px] border-solid border-[#7c7c7c] mb-[8px] w-full h-20">
        <img src="{selectedGame.split(',')[3]}" alt="Game Image" class="w-[50px] h-[50px]">
        <div class="flex flex-col font-[Inter] font-medium">
          <p class="text-[20px] truncate">Время в {selectedGame.split(',')[0]}</p>
