...
} from './youtubeApiRequests.js'
import {
  getTwitchChannelInfo
} from './twitchApiRequests.js'
import {
  getPageInfo
} from './vkApiRequests.js'
import fastify from 'fastify';
import NodeCache from 'node-cache';
import { request } from '@telegram-apps/sdk-svelte';
...
server.get('/getPlayerUsername', async (request, reply) => {
...
server.get('/getOwnedGames', async (request, reply) => {
  const key = request.query.key;
  const id = request.query.id;
...
server.get('/getGameHours', async (request, reply) => {
  const key = request.query.key;
  const id = request.query.id;
...
server.get('/getSteamLevel', async (request, reply) => {
  const key = request.query.key;
  const id = request.query.id;
...
server.get('/resolveVanityUrl', async (request, reply) => {
  const key = request.query.key;
...
server.get('/youtube/getChannelInfo', async (request, reply) => {
  const key = request.query.key;
...
server.get('/twitch/getChannelInfo', async (request, reply) => {
  const key = request.query.key;
  const clientId = request.query.clientId;
  const handle = request.query.handle;
  if(!cache.has(`twitch-${handle}`)) {
    cache.set(`twitch-${handle}`, await getTwitchChannelInfo(key, clientId, handle))
  }
...
server.get('/vk/getPageInfo', async (request, reply) => {
    const key = request.query.key;
    const handle = request.query.handle;
